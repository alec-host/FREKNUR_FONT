	...
	<script type="text/javascript">
		...
		$(document).ready(function () {			
			...
			$('#DataViewTable').jtable({
				...
				actions: {
					listAction: 'loan_request_fetch.php?action=list&jtStartIndex=0&jtPageSize=10&jtSorting=null'
				},
				fields: {
						id: {
							key: true,
					    ...
					    task_flag: {
							...
							edit: false,
							display:function(data){
								if(data.record.task_flag == '0'){
									return '<span id="gridSpan"><i>Waiting for Manual Approval</i></span>';
								}else if(data.record.task_flag == '1'){
									return '<span id="gridSpan"><i>Waiting for System Approval</i></span>';
								}else if(data.record.task_flag == '2'){
									return '<span id="gridSpan"><i>Waiting for Dispatch</i></span>';									
								}else{
									return '<span id="gridSpan"><i>Complete</i></span>';
								}
							}							
						},
						requested_by: {
						...
						btn_action: {
							...
							display: function(data){
								...
								if(data.record){
									if(data.record.task_flag == '1' || data.record.task_flag == '2'){
										c1 = '#777';
										c2 = '#777';	
									}else{
										c1 = '#3DCE00';
									...
									s1 = 'style="background-color:'+c1+';color:#fff;border-radius:2px;border:1px solid #000;font-family:calibri;width:70px"';
									s2 = 'style="background-color:'+c2+';color:#fff;border-radius:2px;border:1px solid #000;font-family:calibri;width:70px"';									
									
									if(data.record.task_flag == '1' || data.record.task_flag == '2'){
										var btn_1 = '<button id="btn_1" name="btn_1" onclick="_dialog(msg)" '+s1+' disabled><span><b>APPROVE</b></span></button>';
										var btn_2 = '<button id="btn_2" name="btn_2" '+s2+' disabled><b>REJECT</b></button>'; 
									}else{
										var btn_1 = '<button id="btn_1" name="btn_1" onclick="_dialog(msg)" '+s1+' ><span><b>APPROVE</b></span></button>';
										var btn_2 = '<button id="btn_2" name="btn_2" '+s2+'><b>REJECT</b></button>'; 									
									}
								return btn_1+" "+btn_2;
								}
